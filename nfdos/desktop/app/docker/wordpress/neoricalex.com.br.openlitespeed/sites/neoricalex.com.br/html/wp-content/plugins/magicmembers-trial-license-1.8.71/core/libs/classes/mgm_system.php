...
class mgm_system extends mgm_object{
	...
	function _set_defaults(){
		...
		$this->setting['aws_region'] 	  = 'us-east-1';	
		$this->setting['aws_signature_version'] = 'v2';
		...
		switch($name){
			...
			case 'login_errmsg_multiple_logins':
				...
				if( $parse ){		
					...
					// action
					$q_args['action'] = ( isset($data['action']) ) ? $data['action'] : '[[ACTION]]';
					
					// token
					if( isset($data['__utoken__']) ){
						$q_args['__utoken__'] = $data['__utoken__'];// user
					}elseif( isset($data['__ptoken__']) ){	
						$q_args['__ptoken__'] = $data['__ptoken__'];// payment
					}else{
						$q_args['usertoken'] = '[[USERTOKEN]]';// user default
					}
	...
	function get_custom_pages_url(){
		// init 
		$custom_pages_url = array(
			'register_url','profile_url','transactions_url','login_url','lostpassword_url',
