...
add_action( 'show_user_profile'                          , 'mgm_admin_user_signup_links', 10, 1);// user signup links
add_action( 'edit_user_profile'                          , 'mgm_admin_user_signup_links', 10, 1);// user signup links
add_action( 'show_user_profile'                          , 'mgm_show_custom_fields', 11, 1 );// user profile custom fields display
add_action( 'edit_user_profile'                          , 'mgm_edit_custom_fields', 11, 1 );// user profile custom fields display/edit
add_action( 'show_user_profile'                          , 'mgm_admin_user_unsubscribe', 12, 1);// user unsubscribe
add_action( 'edit_user_profile'                          , 'mgm_admin_user_unsubscribe', 12, 1);// user unsubscribe
add_action( 'show_user_profile'                          , 'mgm_admin_user_payment_history', 13, 1);// user payment history
add_action( 'edit_user_profile'                          , 'mgm_admin_user_payment_history', 13, 1);// user payment history
add_action( 'user_new_form'                              , 'mgm_new_custom_fields', 10, 1 );//add new user hook after wp3.7
// add_filter('login_redirect'                           , 'mgm_login_form_redirect', 1); // // login redirect deprecated
...
add_filter( 'mgm_module_rebill_status'  			     , 'mgm_module_rebill_status', 10, 3);
...
// redirect
add_action( 'mgm_user_register_after'                     , 'mgm_user_register_redirect', 10, 2 );

//add_filter('mgm_login_err_subscription_url'              , 'mgm_login_err_subscription_url', 10, 2);
...
function mgm_generate_register_form_html($form_template, $form_params){
	...
	$form_html .= '
		<script type="text/javascript">jQuery(document).ready(function(){
			try{
				mgm_date_picker(".mgm_date", false, {yearRange:"'.$yearRange.'", dateFormat: "'. mgm_get_datepicker_format() .'"});
			}catch(x){
			}
		});
		</script>';	
	...
	if(!empty($package) || !empty($membership)) {
		//i ssue#: 482		
		$form_html .= '<script type="text/javascript">jQuery(document).ready(function(){jQuery(\'#registerform\').attr(\'action\',\''.$form_action.'\')});</script>';
	}	
...
function mgm_register($user_id){	
	...
	// we just add the hook and redirect there
	do_action('mgm_user_register_after', $user_id, $redirect);
 ...
 * redirect to payment
 */ 
function mgm_user_register_redirect($user_id, $redirect){
	...
	// redirect	
	if( $redirect !== FALSE ){
		// replace all insecure param to secure
		$redirect = mgm_secure_query_string($redirect, $user_id);
		// check
		// $redirect = add_query_arg(array('usertoken'=>mgm_get_auth_token($user_id)), $redirect);

		// echo 'redirecting to: '.$redirect; exit();

		// mgm_log($redirect, __FUNCTION__);
		...
		// this goes to payments, core/functions/mgm_payment_button_functions.php/mgm_get_subscription_buttons()
 ...
 * secure query string
 */ 
function mgm_secure_query_string( $url, $user_id=null, $encode_uri=true ){
	...
	if( isset($parts['query']) ){
		...
		$ignore = array('username','email','userid', 'user_id'); 
		$query_new  = array();
		...
		foreach($query as $key=>$value){
			...
			if( ! in_array($key, $ignore ) ){
				...
				if( ! $user_id ){
					if( $user = mgm_get_user_by($key, $value) ){
						$user_id = $user->ID;
					}
				}
		...
		if( $add_user_token ){
			$query_new['usertoken'] = mgm_get_auth_token($user_id);
		}

		$port = isset($parts['port']) ? ':'.$parts['port'] : '';
		$url = $parts['scheme'] .'://'.$parts['host'].$port.$parts['path'].'?'._http_build_query( $query_new, null, '&', '', $encode_uri );
	}

	return apply_filters('mgm_secure_query_string', $url);
}
...
function mgm_authenticate_user($user, $return=false){
	...
	if ( isset($_REQUEST['pwd']) && ! wp_check_password($_REQUEST['pwd'], $user->user_pass, $user->ID) ) {
		$str = '<strong>ERROR</strong>: The password you entered for the username <strong>%s</strong> is incorrect.';
		return new WP_Error('incorrect_password', sprintf(__($str, 'mgm'), $user_name));
	}	
	...
	$timestamp = apply_filters( 'mgm_expire_check_current_time', $timestamp, $user->ID, $member);
	...
	if (in_array($member->status, $login_allowed_statuses)) {
		...
		// check expire
		if (!empty($member->expire_date) && $timestamp > strtotime($member->expire_date)) {
			// old status
			...
			// str
			$str = 'Subscription changed to %s by expire date check at login, expire date: %s, now: %s';
			// status string                
			$status_str = sprintf( $str, $new_status, $member->expire_date, date('Y-m-d', $timestamp) );			
			// status string
		...
		if(isset($member->other_membership_types) && is_array($member->other_membership_types) && !empty($member->other_membership_types) ) {
			...
			foreach ($member->other_membership_types as $key => $mem_obj) {
				...
				if(is_numeric($mem_obj->pack_id) && in_array($mem_obj->status, $login_allowed_statuses)){
					// check for expiry
					if ( !empty($mem_obj->expire_date) && $timestamp > strtotime($mem_obj->expire_date)) {
						...
						//issue#2511 	                
						$status_str = sprintf('Subscription changed to %s by expire date check at login, expire date: %s, now: %s', $new_status, $mem_obj->expire_date, date('Y-m-d', $timestamp) );
						// status string
	...
	if ( isset($_POST['pwd']) && wp_check_password($_POST['pwd'], $user->user_pass, $user->ID) ){
		...
		mgm_set_auth_token($user->ID, 'user_pay_token');
	}
...
function mgm_get_login_error($user, $error_code, $action, $return, $pack=NULL){
	...
	// parse data
	$parse_data = array('action'=>$action);

	// set token
	switch( $action ){
		case 'complete_payment':
			$parse_data['__ptoken__'] = mgm_get_auth_token($user->ID, 'user_pay_token');
		break;
		case 'resubscribe':
			$parse_data['__utoken__'] = mgm_get_auth_token($user->ID, 'user_auth_token');
		break;
	}

	// error
	$error_messages = array(
		MGM_STATUS_NULL          => mgm_get_system_template('login_errmsg_null', $parse_data, true),
		MGM_STATUS_TRIAL_EXPIRED => mgm_get_system_template('login_errmsg_trial_expired', $parse_data, true),
		MGM_STATUS_EXPIRED       => mgm_get_system_template('login_errmsg_expired', $parse_data, true),
		MGM_STATUS_PENDING       => mgm_get_system_template('login_errmsg_pending', $parse_data, true),
		MGM_STATUS_CANCELLED     => mgm_get_system_template('login_errmsg_cancelled', $parse_data, true),
		'ANY'                    => mgm_get_system_template('login_errmsg_default', $parse_data, true),
		'date_range'             => mgm_get_system_template('login_errmsg_date_range', $parse_data, true),
		'multiple_logins'        => mgm_get_system_template('login_errmsg_multiple_logins', $parse_data, true),
	);
	...
	// add id in url, deprected #2741
	/*if( bool_from_yn($system_obj->setting['enable_email_as_username']) ){
		$q_args	= array_merge($q_args, array('[[USERID]]'=>mgm_encode_id($user->ID) ));
	}else{
		$q_args	= array_merge($q_args, array('[[USERNAME]]'=>mgm_encode_id($user->user_login) ));	
	}*/

	//echo $action;
	//echo $error_code; die;

	/*if( 'complete_payment' == $action ){
		$q_args	= array_merge($q_args, array('[[USERTOKEN]]' => mgm_get_auth_token($user->ID, 'user_pay_token') ));
		//$error_message
	}else{
		$q_args	= array_merge($q_args, array('[[USERTOKEN]]' => mgm_get_auth_token($user->ID) ));
	}*/
...
function mgm_login_redirect_url($user){
	...
	if ($redirect_url = $system_obj->get_setting('login_redirect_url')) {			
		...
		if(!empty($user->user_login)) {
			...
			$redirect_url = mgm_secure_query_string($redirect_url, $user->ID);
		}
		// redirect								
 ...
 * @return boolean
 */
function mgm_module_rebill_status($user_id, $member=NULL, $context='auto') {	
	// object
	if( ! isset($member) && ! is_object($member)) $member = mgm_get_member($user_id); 
	
	// track
	mgm_update_payment_check_state($user_id, $context);
	...
	if( ! isset($module_obj->code) ){
		...
		// log
		mgm_log( sprintf('User[%s] rebill check error: invalid module - %s', $user_id, $module), __FUNCTION__);
		// return
		return false;
	}	

	// do not process if user is awaiting cancel #1259
	// #2757, process awaiting cancel
	if( $member->status == MGM_STATUS_AWAITING_CANCEL ){
		// check awaiting cancel rebill allowed, for module that supports reactivate
		if ( ! $module_obj->is_rebill_status_check_supported(MGM_STATUS_AWAITING_CANCEL) ){
			// time
			$timestamp = mgm_get_current_datetime(null, false, 'timestamp');
			...
			if( isset($member->status_reset_on) ){
				...
				if( $timestamp < strtotime( $member->status_reset_on ) ){
					// log
					mgm_log( sprintf('User[%s] rebill check error: timestamp less than status_reset_on', $user_id), __FUNCTION__);
					// return
					return false;	
				}
			}	
	...
	if ( ! $module_obj->is_rebill_status_check_supported() ){
		// log
		mgm_log( sprintf('User[%s] rebill check error: rebill status check not supported', $user_id), __FUNCTION__);
		// return
		return false;	
	}

	// do not use expire date delay when manual rebill check triggered
	$skip_manual = false;
	...
	if( isset($member->expire_date) && ! empty($member->expire_date) && ! $skip_manual ){
		// && ! $module_obj->is_rebill_status_check_delayed()
		$timestamp = mgm_get_current_datetime(null, false, 'timestamp');
		// log
		mgm_log( sprintf('User[%s] rebill check debug: timestamp actual: %s', $user_id, date('Y-m-d H:i:s', $timestamp)), __FUNCTION__);
		
		// delayed
		$timestamp = $module_obj->get_rebill_status_check_delay($timestamp);
		// log
		mgm_log( sprintf('User[%s] rebill check debug: timestamp delayed: %s', $user_id, date('Y-m-d H:i:s', $timestamp)), __FUNCTION__);
		...
		if( $current_date <= $expire_date ){
			// log
			mgm_log( sprintf('User[%s] rebill check error: current_date less than or equal to expire_date ', $user_id), __FUNCTION__);
			// return
			return false;
		}
	}
	...
	if( ! isset($member->last_payment_check) || (isset($member->last_payment_check) && $member->last_payment_check != 'disabled') ){
		// log
		mgm_log( sprintf('User[%s] rebill check debug: last stage query_rebill_status call', $user_id), __FUNCTION__);
		// query api, triggered by MGM, 
		// module::query_rebill_status() will be triggered via module::process_status_notify() web hook triggered by gateway	
		...
		// log
		mgm_log( sprintf('User[%s] rebill check error: last_payment_check fail', $user_id), __FUNCTION__);
	}	
...
function mgm_pre_authenticate_user($user_id){
	...
	if($member->status != MGM_STATUS_ACTIVE || !isset($member->last_payment_check_date) || (isset($member->last_payment_check_date) && $member->last_payment_check_date != date('Y-m-d')) ){ 							
		...
		// login 
		if( apply_filters('mgm_module_rebill_status', $user_id, $member, 'login') ){
			$checked = true;
		}	
		// update
	...
	if($user = get_user_by($key, $key_value)){			
		...
		if ( ! is_super_admin($user->ID) )  {
			...
			if($member->status == MGM_STATUS_NULL && empty($member->last_pay_date)){
				...
				$args = array_merge($args, array('usertoken'=>mgm_get_auth_token($user->ID)) );
 ...
 */
 function mgm_expire_check_current_time_delay( $time, $user_id, $member ){
	...
	if( isset($module_obj->code) ){
		// check
		if( $time_delayed = $module_obj->get_rebill_status_check_delay($time) ){
			// check
			if( $time_delayed !== false ){	
				// log
				$msg = sprintf('module: %s, user_id: %d, expire_date: %s, current_time: %s, delayed_current_time: %s', 
					  $member->payment_info->module, $user_id, $member->expire_date, 
					  date('Y-m-d H:i:s', $time), date('Y-m-d H:i:s', $time_delayed));
				// log
				mgm_log( $msg, __FUNCTION__ );
			
				// reset
				$time = $time_delayed;// negative hours
			}			
		}				
	}	
  ...
  * @todo
  */
 /*function mgm_login_err_subscription_url($subscription_url, $q_args){
 	return mgm_secure_query_string($subscription_url, null, false);
 }  */
