...
class mgm_member extends mgm_object{	
	...
	// construct
	public function __construct($id=0, $fields=false){
		// php4
		$this->mgm_member($id, $fields);
	}
	
    // construct
	public function mgm_member($id=0, $fields=false){
		// parent
		parent::__construct(); 
		// defaults
	...
	// defaults
	public function _set_defaults($id=0, $fields=false){
		// code
		$this->code        = __CLASS__;
		// name
	...
	// set inner object vars
	public function set_payment_info($payment_info) {
		// check
	...
	// set single: key=>value
	public function set_custom_field($name, $value) {		
		// merge
		$this->custom_fields->{$name} = $value;
	}
	
	// copy mgm_member object fields to user object 
	public function merge_fields($user){
		global $wpdb;
		// create
		if(!is_object($user)){
			$user = new stdClass();
		...
		// default
		$custom_fields = $mcf->get_fields_where(array('display'=>array('on_register'=>true,'on_profile'=>true)));
		// loop
	...
	public function fix_expire_date(){
	// fix
	...
	public function _default_fields(){
		// return
		return array(
			'trial_on' => 0, 'trial_cost' => 0.00, 'trial_duration' => 0, 'trial_duration_type' => 'd', 'trial_num_cycles'=>0, 
	...
	public function _update_custom_fields(){	
		// save
		if($this->options){	
			// loop		
	...
	public function get_membership_types(){
		...
		$membership_types = array($this->membership_type);

		if( isset($this->other_membership_types) && !empty($this->other_membership_types) ){
			foreach ($this->other_membership_types as $key => $member_oth){
				
				$o_mgm_member = mgm_convert_array_to_memberobj($member_oth, $this->id);

				$membership_types[] = $o_mgm_member->membership_type;
