		...
		jQuery("select[name='search_field_name']").bind('change',function() {	
			...
			switch(jQuery(this).val()){
				case 'membership_type':
					...
					<?php foreach(mgm_get_all_membership_type() as $membership_type):?>
						s.options[s.options.length]=new Option('<?php echo $membership_type['name']?>','<?php echo $membership_type['code']?>',false,<?php echo ($data['search_field_value']==$membership_type['code']?'true':'false');?>);
					<?php endforeach?>
				...
				case 'pack_id':
					...
					var s=document.createElement('select');
						s.name='search_field_value';						
					<?php foreach(mgm_get_all_subscription_package() as $subscription_package):?>
						s.options[s.options.length]=new Option('<?php echo strip_tags(mgm_get_subscription_package_description(null, $subscription_package))?>','<?php echo $subscription_package['id']?>',false,<?php echo ($data['search_field_value']==$subscription_package['id']?'true':'false');?>);
					<?php endforeach?>
					jQuery('#fld_wrapper').html(s);
				break;
				case 'status':
