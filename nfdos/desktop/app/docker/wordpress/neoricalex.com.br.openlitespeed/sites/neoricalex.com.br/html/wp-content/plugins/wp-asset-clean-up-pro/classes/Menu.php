...
HTML;
	...
	public function pluginPagesAccessDenied()
	{
		...
		$userCanActivatePlugins = current_user_can('activate_plugins')
			? '<strong style="color:green;">Yes</strong>'
			: '<strong style="color: #cc0000;">No</strong>';

		wp_die(
			__('Sorry, you are not allowed to access this page.').'<br /><br />'.
			sprintf(__('Asset CleanUp requires "%s" role and the ability to activate plugins in order to access its pages.', 'wp-asset-clean-up'), '<span style="font-weight: bold;">'.self::getAccessCapability().'</span>').'<br />'.
				'<ul>'.
					'<li>'.sprintf(__('Your current role(s): <strong>%s</strong>', 'wp-asset-clean-up'), implode(', ', $userRoles)).'</li>'.
					'<li>'.sprintf(__('You have the capability to activate plugins: %s', 'wp-asset-clean-up'), $userCanActivatePlugins).'</li>'.
				'</ul>'.
			...
			'<p style="margin: 0;"><code style="background: #f2f3ea; padding: 5px;">add_filter(\'wpacu_access_role\', function($role) { return \'your_role_here\'; });</code></p>'.
