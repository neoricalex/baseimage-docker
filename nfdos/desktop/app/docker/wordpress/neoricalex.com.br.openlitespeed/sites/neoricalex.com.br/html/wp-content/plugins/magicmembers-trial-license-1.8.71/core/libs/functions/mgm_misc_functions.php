 ...
 */
function mgm_remote_get($request_url, $data=null, $options=array(), $error_message ='failed to connect', $status_codes=null){
 	// args
	$args = array();
	...
	// check
	if( defined('DEBUG_REMOTE_GET') ){
	// log
		mgm_log( 'args: '. mgm_pr($args, true), __FUNCTION__ );
	}
	...
	if( defined('DEBUG_REMOTE_GET') ){
		// log
		mgm_log( 'request: '. mgm_pr($request,true), __FUNCTION__ );
	}
	...
	if( is_wp_error( $request ) ){
		...
		if( is_null($status_codes) ){
			$allowed_status_codes = array(200, 302);
		}else{
			$allowed_status_codes = $status_codes;
		}

		// filter status
		if( is_array($allowed_status_codes) && ! empty($allowed_status_codes) ){
			$status_code = wp_remote_retrieve_response_code( $request );
			if( ! in_array($status_code, $allowed_status_codes) ){
				if($error_message !== FALSE){
					$error = __($error_message, 'mgm');
				}else{
					$error = sprintf( __('Status Code %d returned from server.', 'mgm'), $status_code );
				}
			}
		}
	}	
 ...
 * @return mixed $response
 */
function mgm_remote_post($request_url, $data=null, $options=array(), $error_message='failed to connect', $status_codes=null){
	// args
	$args = array();

	// body
	if( ! is_null($data) ){
		$args = array_merge($args, array('body' => $data));
	}
 ...
 * @return mixed $response
 */
function mgm_remote_head($request_url, $data=null, $options=array(), $error_message ='failed to connect', $status_codes=null){
 	// args
	$args = array();
 ...
 * @return mixed $response
 */ 
function mgm_remote_delete($request_url, $data=null, $options=array(), $error_message ='failed to connect', $status_codes=null) {
	// fetch
	...
	// check
	if( defined('DEBUG_REMOTE_DELETE') ){
		// log
		mgm_log( 'args: '. mgm_pr($args, true), __FUNCTION__ );
	}
 ...
 */
function mgm_get_user_by($key, $value){
	$params = array('username'=>'login', 'email'=>'email','user_id'=>'id','userid'=>'id');

	foreach( $params as $get_var => $user_var ){
		switch( $get_var ){
			case 'username':
	...
	//echo '<br>user_var:'. $user_var.' value: '. $value;

	return $user = get_user_by($user_var, $value); 
}
 ...
 * @todo use trans_ref to fetch too
 */
function mgm_get_user_by_querystring($encoded=false){
	// init
	$user = false;

	// parems
	$params = array(
		'usertoken'  => '_mgm_user_auth_token',
		'__utoken__' => '_mgm_user_auth_token',
		'__ptoken__' => '_mgm_user_auth_token',
	);

	foreach( $params as $get_var => $user_var ){
		// check
		if( isset($_GET[$get_var]) && !empty($_GET[$get_var]) ){
			// get
			$value = $_GET[$get_var];

			// decode
			if( $encoded ){
				$value = mgm_decode_id($value);
			}
			...
			switch( $get_var ){
				...
				case '__ptoken__':
					// name
					$key = ('__ptoken__' == $get_var) ? '_mgm_user_pay_token' : '_mgm_user_auth_token';
					
					// sanitized
					$token_sanitized = sanitize_meta( $key, $value, 'user' );

					// find args
					$args = array('meta_key'=>$key,'meta_value'=>$token_sanitized,'meta_compare'=>'=');
			...
			// check
			if( ! isset($user) ){
				$user = get_user_by($user_var, $value ); 	
			}
...
function mgm_save_partial_fields($display=NULL, $name='mgm_upgrade_field', $cost, $is_single=true, $action = 'upgrade', $member = null){
	...
	if($user->ID == 0 || !is_numeric($user->ID)) {
		$user = mgm_get_user_by_querystring();						
	}
	// error
...
function mgm_get_upgrade_link() {
	...
	// upgrade_url
	$upgrade_url = mgm_get_custom_url('transactions', false, 
		array('action'=>'upgrade','usertoken'=>mgm_get_auth_token($user->ID),'upgrade_prev_pack'=>$pack_id));
	// link_label
...
function mgm_get_extend_link() {
	...
	$pack_id = $member->pack_id;		
	// extend url
	$extend_url = mgm_get_custom_url('transactions',false,
		array('action'=>'extend','usertoken'=>mgm_get_auth_token($user->ID),'pack_id'=>$pack_id));
	// link_label
...
function mgm_download_parse($content) {
	...
	if (substr_count($content,"[" . $hook . "#")) {
		// sql
		$sql = 'SELECT `id`, `title`, `filename`, `post_date`, `members_only`, `user_id`, `code` 
		        FROM `' . TBL_MGM_DOWNLOAD . '`';
		...
		if ( $downloads = $wpdb->get_results($sql) ) {
			...
			foreach($downloads as $download) {
				...
				// link tag
				$tag = "[" . $hook . "#" . $download->id . "]";

				// match
				if( preg_match('/'.preg_quote($tag, '/').'/', $content) ){
					// Download link
					$link = '<a href="' . $download_url . '" title="' . $download->title 
					        . '" >' . $download->title . '</a>';
					// replace
				...
				// image tag
				$tag = "[" . $hook . "#" . $download->id . "#image]";

				// match
				if( preg_match('/'.preg_quote($tag, '/').'/', $content) ){
					// image
					$download_image_button = apply_filters('mgm_download_image_button', 
						sprintf('<img src="%s" alt="%s" />',MGM_ASSETS_URL . 'images/download.gif', $download->title), 
						$download->title);

					// Image link
					$link = '<a href="' . $download_url . '" title="'.$download->title
					        .'">'.$download_image_button.'</a>';
					// replace
					$content = str_replace($tag, $link, $content);
				}
					
				// button tag
				$tag = "[" . $hook . "#" . $download->id . "#button]";

				// match
				if( preg_match('/'.preg_quote($tag, '/').'/', $content) ){
					// Button link
					$link = '<input type="button" name="btndownload-'.$download->id
					        .'" onclick="window.location=\''.$download_url.'\'" title="'
					        .$download->title.'" value="'.__('Download','mgm').'"/>';
 ...
 * @since 1.8.65
 */ 
function mgm_get_aws_regions(){
	// Code => Name
	return $aws_regions = array(
		'us-east-1' => __('US East (N. Virginia)', 'mgm'),
		'us-east-2' => __('US East (Ohio)', 'mgm'),
		'us-west-1' => __('US West (N. California)', 'mgm'),
		'us-west-2' => __('US West (Oregon)', 'mgm'),
		'ca-central-1' => __('Canada (Central)', 'mgm'),
		'eu-west-1' => __('EU (Ireland)', 'mgm'),
		'eu-central-1' => __('EU (Frankfurt)', 'mgm'),
		'eu-west-2' => __('EU (London)', 'mgm'),
		'ap-northeast-1' => __('Asia Pacific (Tokyo)', 'mgm'),
		'ap-northeast-2' => __('Asia Pacific (Seoul)', 'mgm'),
		'ap-southeast-1' => __('Asia Pacific (Singapore)', 'mgm'),
		'ap-southeast-2' => __('Asia Pacific (Sydney)', 'mgm'),
		'ap-south-1' => __('Asia Pacific (Mumbai)', 'mgm'),
		'sa-east-1' => __('South America (São Paulo)', 'mgm'),
	);	
}

/**
 * get aws signature versions
 */ 
function mgm_get_aws_signature_versions(){
	return $aws_signature_versions = array(
		'v2' => __('Signature Version 2', 'mgm'),
		'v4' => __('Signature Version 4', 'mgm'),
	);
}	
...
function mgm_get_s3file_info($fileurl){
	...
	$aws_region = $system_obj->get_setting('aws_region', 'us-east-1');
	$aws_signature_version = $system_obj->get_setting('aws_signature_version', 'v2');
	
	// sdk version
	if( ! defined('MGM_AWSSDK_VERSION') ){
		// for v4 use latest
		if( 'v4' == $aws_signature_version ){
			define('MGM_AWSSDK_VERSION', '3.30.1');
		}else{
			define('MGM_AWSSDK_VERSION', '1.6.2'); // '1.4.1'
		}
	}
	
	// version
	switch( MGM_AWSSDK_VERSION ){
		case '1.4.1':// first 
			// Include the SDK
			require_once MGM_LIBRARY_DIR . 'third_party/awssdk/v'.MGM_AWSSDK_VERSION.'/sdk.class.php';// 1x series
			// s3 object
		...
		break;
		case '3.30.1':// sdk latest, better with composer 
			require_once MGM_LIBRARY_DIR . 'third_party/awssdk/v'.MGM_AWSSDK_VERSION.'/aws-autoloader.php';
...
function mgm_download_s3file($fileurl){
	...
	$localfile = MGM_FILES_DOWNLOAD_DIR . 's3file_' . time() . '_' . basename($s3_info->filename);
 	...
 	// download
 	if( version_compare(MGM_AWSSDK_VERSION, '3.0.0', '>=') ){
	 	try{
	 		$args = array(
			    'Bucket' => $s3_info->bucket,
			    'Key' => $s3_info->filename,
			    'SaveAs' => $localfile
			);

			$result = $s3_info->s3->getObject($args);

			// success
			if( $result->get('ContentLength') ){
				return $localfile;
			}	
...
function mgm_get_s3file_size($fileurl){
	...
	if( version_compare(MGM_AWSSDK_VERSION, '3.0.0', '>=') ){
		...
		try{
			...
			$result = $s3_info->s3->headObject($args);
		...
		$size = $s3_info->s3->get_object_filesize($s3_info->bucket, $s3_info->filename);
	}	
...
function mgm_get_s3token_url($fileurl, $expires='1 HOUR'){
	...
	if( version_compare(MGM_AWSSDK_VERSION, '3.0.0', '>=') ){
		try{
			...
			// command
			$cmd = $s3_info->s3->getCommand('GetObject', $args);

			// request
			$request = $s3_info->s3->createPresignedRequest($cmd, $expires);

			// token url
			$token_url = (string) $request->getUri();
		}catch( Exception $e ){
			mgm_log( $e->getMessage(), __FUNCTION__ );
		...
		$token_url = $s3_info->s3->get_object_url($s3_info->bucket, $s3_info->filename, $expires);
	}
 ...
 function mgm_get_s3torent_url($fileurl, $expires='1 HOUR'){
	...
	if( version_compare(MGM_AWSSDK_VERSION, '3.0.0', '>=') ){
		try{
			...
			// command 
			$cmd = $s3_info->s3->getCommand('GetObjectTorrent', $args);

			// request
			$request = $s3_info->s3->createPresignedRequest($cmd, $expires);

			// torent
			$torrent_url = (string) $request->getUri();
		...
		$torrent_url = $s3_info->s3->get_torrent_url($s3_info->bucket, $s3_info->filename, $expires);	
	}
...
function mgm_user_has_access($post_id = false, $allow_on_purchasable = false) {
	...
	// get user by rss token, only for feed	
		$user = mgm_get_user_by_rss_token(strip_tags($_GET['token']));	
	} else {
...
function mgm_member_unsubscribe(){
	...
	if($post_membership_type = $membership_type && $member->membership_type != $post_membership_type){
 ...
 * @since 1.0
 */
function mgm_get_user_by_rss_token($token) {
	$meta_exists = true;
	// get users by meta :
...
function mgm_edit_custom_fields($user_ID=false, $submit_row=false, $return=false, $field_prefix='mgm_profile_field') {
	...
	if (count($fields)) {
		...
		$html .= '<script type="text/javascript">jQuery(document).ready(function(){try{mgm_date_picker(".mgm_date",false,{yearRange:"'.$yearRange.'", dateFormat: "'.mgm_get_datepicker_format().'"});}catch(x){}});</script>';
	}
...
function mgm_save_custom_fields($user_id=NULL, $field_prefix='mgm_profile_field') {
	...
	if (isset($_POST['submit'])) {		
		...
		foreach($cf_on_profilepage as $field){
			...
			if(isset($_POST[$field_prefix][$field['name']])){				
			...
			}else if(isset($_POST[$field_prefix][$field['name']]) && isset($_POST[$profile_fields[$field['name']]['name']]) 
				&& isset($profile_fields[$field['name']]) && isset($profile_fields[$field['name']]['name'])){	
				// value as it was posted
				...
				if($field['name'] == 'birthdate') {	
					$value = mgm_format_inputdate_to_mysql($value); 					
				}elseif ($field['name'] == 'password' && isset($_POST['pass1']) && !empty($_POST['pass1'])) {
					// encode for password
...
function mgm_no_access_redirect($system_obj) {	
	...
	if (!empty($url)) {
		if (!headers_sent()) {
			...
			return sprintf('<script type="text/javascript">document.location="%s";</script>', $url);
		}
	}
 ...
 * @since 1.5.0
 */
function mgm_get_jqueryui_version($force=false){
	if( $force ){
		delete_option('mgm_jqueryui_version');
	}
...
if( ! function_exists('mgm_get_system_template') ){
	/**
	 * get system template
	 * 
	 */ 
	function mgm_get_system_template($name, $data=array(), $parse=false){
		return mgm_stripslashes_deep ( mgm_get_class('system')->get_template($name, $data, $parse) );
	}
}
...
function mgm_sidebar_register_links($username=false, $return=false, $template='sidebar') {
	...
	foreach ($packs as $pack) {
		...
		if( !isset($pack['hidden']) || (isset($pack['hidden']) && $pack['hidden'] != 1) ){
...
function mgm_get_userdata($user_id){
	...
	foreach($profile_fields as $name => $field){
		if( isset($field['name']) ){
			...
			if(isset($user->{$field['name']}) && empty($user->{$field['name']})){
				// check mgm if set
...
function mgm_process_user_login($action=NULL, $user_login=NULL, $user_password=NULL) {
	...
	$custom_fields = mgm_get_class('member_custom_fields')
		->get_fields_where(array('display'=>array('on_login'=>true,'on_login_widget'=>true),'name'=>'captcha'));
...
function mgm_check_post_access_delay($member, $user, $access_delay) {
	...
	// pref	
	$post_delay_preference = mgm_get_setting('post_delay_preference');	
...
function mgm_api_access_allowed(){
	// check if disabled
	if(mgm_get_setting('rest_server_enabled') == 'N') return false;
...
function mgm_get_current_datetime($format = 'Y-m-d', $format_timestamp = true, $key=null) {
	...
	if( ! is_null($format) ){
		...
		if( $format_timestamp ){
			$return['timestamp'] = strtotime($return['date']);
		}
	}else{
...
function mgm_redirect($location, $status = 302, $type='header', $return=false){	
	...
	switch($type){		
		case 'javascript':			
			// only if no headers
			$header = sprintf('<script type="text/javascript">window.location = "%s";</script>', $location); 
			// return
...
function mgm_get_css_group($section='site'){
	// load
	if(!$css_group = mgm_get_setting('css_settings')){
		$css_group = 'default';
	} 
	// return 
	return $css_group;	
		...
		if($id = mgm_decode_id(strip_tags($_GET['trans_ref']))){
			...
			if( mgm_auto_login($id) ){ 
				...
				if(!headers_sent()){ 				
					...
					return sprintf('<script type="text/javascript">window.setTimeout(function(){window.location.href="%s";}, %d)</script>', strip_tags($_GET['redirect_to']), 5 * 5);	
				}
				// exit;	
				exit;
			}	
...
if( ! function_exists('mgm_get_auth_token') ){
	/**
	 ...
	 * @since 1.0
	 */
	function mgm_get_auth_token( $user_id = null, $name='user_auth_token' ) {	
		global $wpdb;
		...
		// set 
		return mgm_set_auth_token($user_id, $name);
	}
}	

if( ! function_exists('mgm_set_auth_token') ){
	/**
	 * get or generate auth token for user
	 *
	 * @param integer $user_id
	 ...
	 * @since 1.0
	 */
	function mgm_set_auth_token( $user_id = null, $name='user_auth_token', $reset=false ) {	
		global $wpdb;
		
		// check
		if ( $user_id ) {
			// key
			$auth_key = sprintf('_mgm_%s', $name);// _mgm_user_auth_token | _mgm_user_pay_token
			$auth_expire_key = $auth_key.'_expire_at';

			// reset
			if( $reset ){
				delete_user_option($user_id, $auth_key, true);
				delete_user_option($user_id, $auth_expire_key, true);
			}
			
			// check
			if( ! $auth_token = get_user_option($auth_key, $user_id) ){
				// token
				$auth_token = mgm_create_auth_token( 'mgm'.str_replace('_', '', $name) );

				// also update usermeta
				update_user_option($user_id, $auth_key, $auth_token, true);

				// timestamp
				$timestamp = mgm_get_current_datetime(null, false, 'timestamp');

				// expire usermeta
				update_user_option($user_id, $auth_expire_key, strtotime('+3 DAY', $timestamp), true);// 3 day expire
				...
				// expire_at
				if( $expire_at = get_user_option($auth_expire_key, $user_id) ){
					// timestamp
					if( $timestamp > $expire_at ){
					// recreate	
						return mgm_set_auth_token( $user_id, $name, true );
					}
				}
			}
...
if( ! function_exists('mgm_create_rss_token') ){
	/**
	 * create rss token
	 *
	 * @param none
	 ...
	 * @since 1.0
	 */
	function mgm_create_rss_token($salt='magicmembersrsstoken') {
		// return
		return mgm_create_secure_token($salt);
	}
}

if( ! function_exists('mgm_create_auth_token') ){
	/**
	 * create auth token
	 *
	 * @param none
	 ...
	 * @since 1.0
	 */
	function mgm_create_auth_token($salt='magicmembersauthtoken') {
		// return
		return mgm_create_secure_token($salt);
	}
}

if( ! function_exists('mgm_create_secure_token') ){
	/**
	 * create auth token
	 *
	 * @param none
	 ...
	 * @since 1.0
	 */
	function mgm_create_secure_token($salt){
...
if( ! function_exists('mgm_create_token') ){
	/**
	 * create token
	 */
...
if( ! function_exists('mgm_get_pack_ref') ){

	/**
	 * get pack ref encoded
	 * 
	 * @since 1.6.67
	 */ 
	function mgm_get_pack_ref($member=null, $amount=null, $duration=null, $duration_type=null, $membership_type=null){
		if( ! is_null($member) ){
			return md5($member->amount .'_'. $member->duration .'_'. $member->duration_type .'_'. $member->membership_type);
		}	

		return md5($amount .'_'. $duration .'_'. $duration_type .'_'. $membership_type);
	}
}

if( ! function_exists('mgm_is_member_payment_status_check_delayed') ){

	/**
	 * check member payment status check delayed
	 * 
	 * @param object $member
	 * @return bool
	 * @since 1.6.68
	 */ 
	function mgm_is_member_payment_status_check_delayed($member){
		// module objcet 
		$module_obj = null;
		...
		// if virtual payment
		if( $module_obj->is_virtual_payment() ){
			return false;
		}	
		...
		if( isset($member->expire_date) && ! empty($member->expire_date) ){
			...
			// delayed timestamp
			$timestamp = $module_obj->get_rebill_status_check_delay($timestamp);
			
			// current date
			$current_date = date('Y-m-d', $timestamp);// if delayed it will be today - hours => in past
...
if( ! function_exists('mgm_is_member_payment_module_virtual') ){

	/**
	 * check member payment module virtual
	 * 
	 * @param object $member
	 * @return bool
	 * @since 1.6.68
	 */ 
	function mgm_is_member_payment_module_virtual($member){
		// module objcet 
		$module_obj = null;
		if( ! empty($member->payment_info->module) ){
			// validate
		...
		return $module_obj->is_virtual_payment();
	}
}	

if( ! function_exists('mgm_get_payment_modules_as_list') ){
	/**
 	 * @todo
 	 */ 	
	function mgm_get_payment_modules_as_list(){
		// module codes
		$module_codes = mgm_get_class('system')->get_active_modules('payment');
		// init
		$modules = array();
		// module tracking fields
		if( !empty($module_codes) ){
			foreach($module_codes as $module_code){
				$modules[$module_code] = mgm_get_module($module_code)->get_name();
			}
		}	
