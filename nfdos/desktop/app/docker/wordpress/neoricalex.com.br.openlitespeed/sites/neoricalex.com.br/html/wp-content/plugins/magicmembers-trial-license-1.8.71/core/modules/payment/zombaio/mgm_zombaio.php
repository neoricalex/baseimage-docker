...
class mgm_zombaio extends mgm_payment{
	...
	function get_button_unsubscribe($options=array(), $show_heading=true, $add_form=true){
		...
		/*// message
		$message = sprintf(__('You have subscribed to <span>%s</span> via <span>%s</span>, to unsubscribe, please click the following link.','mgm'),get_option('blogname'), $this->name);*/
	...
	********************************************************************/
	...
	function _get_button_data($pack, $tran_id=NULL) {
		...
		// $data['extra'] = $tran_id; 
		// passthrough		
		$data['extra'] = '&extra='.$tran_id; // needed as per iss#2741
