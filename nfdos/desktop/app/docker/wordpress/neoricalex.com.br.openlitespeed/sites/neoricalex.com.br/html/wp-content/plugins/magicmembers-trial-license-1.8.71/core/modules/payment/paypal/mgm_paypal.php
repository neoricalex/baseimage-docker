...
class mgm_paypal extends mgm_payment{	
	...
	function get_button_unsubscribe($options=array(), $show_heading=true, $add_form=true){		
				...
				!empty($this->setting['password']) && !empty($this->setting['signature'])) 
		{
			...
			$form_method_action = 'method="post" action="'. $action .'"';
		}else {
			$action = $this->_get_endpoint().'?cmd=_subscr-find&alias='. urlencode($this->setting['business_email']);
			// else send the user to PAYPAL		
			$form_method_action = 'method="get" action="'. $action .'"';
		}
		...
		if( $add_form ){
		   	$html .='<form name="mgm_unsubscribe_form" id="mgm_unsubscribe_form" ' . $form_method_action . ' >	';
		}
