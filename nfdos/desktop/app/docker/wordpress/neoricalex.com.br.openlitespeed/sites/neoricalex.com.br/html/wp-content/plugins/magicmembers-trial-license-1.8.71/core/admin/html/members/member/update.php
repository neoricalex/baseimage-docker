...
<script type="text/javascript" language="javascript">
	...
	jQuery(document).ready(function(){					
		...
		jQuery("#mgmmembersfrm :checkbox[name='update_opt[]']").bind('click', function(){
			...
			var _checked = jQuery(this).prop('checked');
			var _name = jQuery(this).val();

			// switch state			
			jQuery('#upd_elements_'+_name).find(':input').attr('disabled', !_checked);
			// date 
			if( _name == 'expire_date'){
				var _opt = {
					yearRange:"<?php echo mgm_get_calendar_year_range(); ?>", 
					dateFormat: "<?php echo mgm_get_datepicker_format();?>"
				};

				mgm_date_picker("#mgmmembersfrm :input[name='upd_expire_date']",'<?php echo MGM_ASSETS_URL?>', _opt);			
			}		
		});
