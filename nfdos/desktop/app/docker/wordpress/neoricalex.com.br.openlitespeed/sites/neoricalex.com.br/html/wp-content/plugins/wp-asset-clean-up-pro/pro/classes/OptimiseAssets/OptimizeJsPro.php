...
class OptimizeJsPro
{
	...
	{
		add_filter('wpacu_media_queries_load_for_js', function($htmlSource) { return MatchMediaLoadPro::alterHtmlSourceForMediaQueriesLoadJs($htmlSource); });
	}
	...
	public static function getAllInlineChosenPatterns()
	{
		...
		if (strpos($inlineJsFilesPatterns, "\n")) {
			...
			// Only one value?
			$allPatterns[] = preg_quote(trim($inlineJsFilesPatterns), '/');
		}
	...
	public static function doInline($htmlSource)
	{
		...
		if (! empty($matchesSourcesFromTags)) {
			...
			foreach ($matchesSourcesFromTags as $matchList) {
				...
				if (strpos($matchedTag, '/wp-includes/js/admin-bar.js') !== false) {
					continue;
				}
				...
				$chosenInlineJsMatches = (! empty($allPatterns) && preg_match('/(' . implode('|', $allPatterns) . ')/i', $matchedTag));
