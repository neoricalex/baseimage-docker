...
foreach ($allAssets as $obj) {
	...
	*/
	...
	// [wpacu_pro]
	$row['is_load_exception_regex_match'] = $isLoadExceptionRegExMatch;
	// [/wpacu_pro]
	...
	if ( ! $isLoadException && ( $row['is_group_unloaded'] || $isUnloadRegExMatch ) ) {
		// [wpacu_pro]
		if ( $isUnloadRegExMatch ) {
			$row['is_regex_unload_match'] = true;
		}
		// [/wpacu_pro]
...
foreach ($allAssetsFinal as $assetHandle => $row) {
	...
	if (isset($data['rows_build_array']) && $data['rows_build_array']) {
		...
		if (isset($data['rows_by_location']) && $data['rows_by_location']) {
			...
			$handlePosition = (isset($row['obj']->position_new) && $row['obj']->position_new) ? $row['obj']->position_new : $row['obj']->position;
