	...
	jQuery(document).ready(function(){								
		...
		jQuery("#custom_field_rows").sortable({
			update:function(event, ui){						
				...
				// if(!ui.item.children('td').children('input').prop('checked')) return;		
				if(!ui.item.children('div:first').children(":input[name='custom_fields[]']").prop('checked')) return;					
				// post
				jQuery.ajax({
		...
		jQuery("#custom_field_rows :checkbox[name='custom_fields[]']").bind('click',function(){		
			...
			var active   = (jQuery(this).prop('checked'))?'Y':'N';
