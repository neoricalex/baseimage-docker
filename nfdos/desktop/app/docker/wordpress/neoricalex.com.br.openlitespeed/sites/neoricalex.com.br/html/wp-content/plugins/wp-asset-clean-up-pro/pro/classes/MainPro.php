...
class MainPro
{
		...
		*/
		if ( is_404() || Main::isWpDefaultSearchPage() || is_date() || Main::isCustomPostTypeArchivePage() ) {
			...
			if ( empty( $bulkUnloadJson ) || ! ( Misc::jsonLastError() === JSON_ERROR_NONE ) ) {
				return json_encode( array( 'styles' => array(), 'scripts' => array() ) );
			}
	...
	public function currentScreen()
    {
        ...
        if (Main::instance()->settings['hide_assets_meta_box']) {
            return;
        }
	    ...
	    if ($current_screen->base === 'term' && isset($current_screen->taxonomy) && $current_screen->taxonomy !== '') {
		    add_action('admin_head', static function() {
		        ...
		        ?>
                <style data-wpacu-admin-inline-css="1">
                    #edittag {
                        max-width: 96%;
                    }
    ...
    {
        if ($this->scriptsAttributesChecked || Plugin::preventAnyChanges() || Main::instance()->preventAssetsSettings()) {
