...
function nsan_cleanup_tempfiles() {
...
function nscan_cancel_scan() {
	...
	wp_send_json( array( 'status' => 'success', 'message' => __('Scan cancelled', 'ninjascanner' ) ) );
}
...
function nscan_log( $string, $level = 1, $exit = true ) {
	...
	if ( $exit == true ) {
		...
		if ( in_array( $lock_status['status'], array( 'notfound', 'cancelled' ) ) ) {
			file_put_contents(
				NSCAN_DEBUGLOG,
...
function nscan_generate_key() {
	...
	set_transient( 'nscan_ajax_start', hash( 'sha256', $key ), 60 * NSCAN_KEYTIMEOUT );
	return $key;
}
...
function nscan_check_nonce() {
	...
	if ( empty( $_POST['nscan_key'] ) || ! wp_verify_nonce( $_POST['nscan_key'], 'nscan_on_demand_nonce' ) ) {
		$return['status'] = 'error';
...
function nscan_check_key() {
	...
	$error = array(
		'status' 	=> 'error',
		'message'	=> __('Security keys do not match. Try to reload this page.', 'ninjascanner'),
	);
