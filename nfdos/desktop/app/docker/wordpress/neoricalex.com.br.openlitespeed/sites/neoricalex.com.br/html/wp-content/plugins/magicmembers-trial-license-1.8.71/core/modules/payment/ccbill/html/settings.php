		...
		<div class="table">
			...
			<div class="row">
				<div class="cell">
					...
					<p><div class="tips"><?php _e('Enable/Disable Rebill Status Query via CCBill DataLink SMS API.','mgm'); ?></div></p>		
				</div>
			</div>
	...
	jQuery(document).ready(function(){   
		...
		jQuery("#frmmod_<?php echo $data['module']->code?>").validate({
			...
			rules: {			
				...
				'setting[upgrade_enc_key]': {
					required: function(){ return jQuery("select#setting_upgrade_api").val() == 'upgrade'}
				},
				'setting[md5_hashsalt]': {
					required: function(){ return jQuery("select#setting_dynamic_pricing").val() == 'enabled'}
				},
				
				//'setting[datalink_username]': "required",
				//'setting[datalink_password]': "required"

				'setting[datalink_username]': {
					required: function(){
						return jQuery("#frmmod_<?php echo $data['module']->code?>")
					        .find('#setting_rebill_status_query').val() == 'enabled';
				}},
				'setting[datalink_password]': {
					required: function(){
						return jQuery("#frmmod_<?php echo $data['module']->code?>")
					        .find('#setting_rebill_status_query').val() == 'enabled';
				}}
