...
class mgm_moneybookers extends mgm_payment{	
	...
	function get_button_unsubscribe($options=array(), $show_heading=true, $add_form=true){
		...
		// message
		$message = sprintf(__('You have subscribed to <b>%s</b> via <b>%s</b>, to unsubscribe, please click the following link. <br>','mgm'), get_option('blogname'), $this->name);		
		// html
		$html='<div style="margin-bottom: 10px;">
			   ...
			   <form name="mgm_unsubscribe_form" id="mgm_unsubscribe_form" method="post" action="'. $action .'">
					...
					<input type="button" name="btn_unsubscribe" value="' . __('Unsubscribe','mgm') . '" onclick="mgm_confirm_unsubscribe(this, \''.$action.'\')" class="button" />	
			   </form>';	
		// return
		return $html;		
	}	
