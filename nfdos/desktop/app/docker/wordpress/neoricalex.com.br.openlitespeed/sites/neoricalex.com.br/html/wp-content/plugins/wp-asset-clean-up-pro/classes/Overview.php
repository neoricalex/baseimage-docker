	...
	public static function renderHandleTd($handle, $assetType, $data, $for = 'default')
	{
        ...
        if ( $for === 'default' ) {
	        ...
	        if (isset($handleData['media_queries_load']['enable'], $handleData['media_queries_load']['value'])
		        ...
		        ?>
                <div><small><span class="dashicons dashicons-desktop" style="vertical-align: middle;"></span> Downloaded if the media query matches: <code><?php echo ucfirst(htmlspecialchars($data['handles'][$assetType][$handle]['media_queries_load']['value'])); ?></code></small></div>
		        <?php
	        }
