...
class mgm_authorizenet extends mgm_payment{
	...
	function process_credit_card(){			
		...
		// log
		mgm_log('AIM: ' . mgm_pr($this->response, true), $this->get_context( __FUNCTION__ ));
		...
		if((isset($this->response['response_status']) && (int)$this->response['response_status'] == 1) || $bypass_aim === true ) {			
			...
			// log
			mgm_log('recurring: '.$recurring, $this->get_context( __FUNCTION__ ));
			// if recurring
			if($recurring) {
				...
				// log				
				mgm_log('ARB:'. mgm_pr($arb_response, true), $this->get_context( __FUNCTION__ ));
				// check							
	...
	function _process_response($gateway_method, $content, $set_response = true){		
		...
		switch($gateway_method){
			...
			case 'arb_silent_post':
				...
				if(count($content)==0){					
					$temp_resp['response_status'] = 3;
					$temp_resp['message_text']    = __('Error occurred', 'mgm');
				}else{					
					// pos params
	...
	function _fetch_merchant_txn_id(&$member, $force=false){
		...
		if ( ! isset($member->payment_info->txn_id) || empty($member->payment_info->txn_id) || $force) {
			...
			if( ! $force ){
				...
				// log
				mgm_log('fetch from transaction option: '. $authorizenet_transaction_id, $this->get_context( __FUNCTION__ ) );
			}			
			...
			if( ! $authorizenet_transaction_id ){
				...
				// log
				mgm_log('fetch from refetch using batch api: '. $authorizenet_transaction_id, $this->get_context( __FUNCTION__ ) );
			}
	...
	function _fetch_merchant_txn_id_old(&$member, $force=false){
		...
		if (!isset($member->payment_info->txn_id) || empty($member->payment_info->txn_id)) {
			...
			// set	
				$member->payment_info->txn_id = $authorizenet_transaction_id;
				// log
				// mgm_log('fetch from transaction option: '. $member->payment_info->txn_id, $this->get_context( __FUNCTION__ ) );
				// save
				return $member->save();
			}else{
				// // fetch from batch, @todo test before running
				...
				if( (int)$authorizenet_transaction_id > 0 ){
					// set
				 	$member->payment_info->txn_id = $authorizenet_transaction_id;
				 	// log
					// mgm_log('fetch from refetch using batch api: '. $member->payment_info->txn_id, $this->get_context( __FUNCTION__ ) );
				 	// save
				 	return $member->save();
				}
			}
	...
	function get_settled_batches($date_from, $date_to=NULL){
		...
		if($xml_o = @simplexml_load_string($http_response)){
			// log
			// mgm_log('batches response:' . mgm_pr($xml_o, true), $this->get_context( __FUNCTION__ ) );
			// check
			...
			// log
			// mgm_log('batches: '. mgm_pr($batches, true), $this->get_context( __FUNCTION__ ) );		
		}
		// return
		return $batches;
	}
	...
	function get_batch_transactions($batches){	
		...
		foreach($batches as $batchId){
			...
			if($xml_o = @simplexml_load_string($http_response)){
				// log
				// mgm_log('batch transactions response:' . mgm_pr($xml_o, true), $this->get_context( __FUNCTION__ ) );
				// check
				...
				// log
				// mgm_log('batch transactions: '. mgm_pr($transactions, true), $this->get_context( __FUNCTION__ ) );
			}
		}
	...
	function get_positional_fields($content){
		//log		    
		mgm_log('status : '.$this->status, $this->get_context( __FUNCTION__ ) );					    	
		...
		// log
		mgm_log($temp, $this->get_context( __FUNCTION__ ) );
