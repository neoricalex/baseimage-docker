...
function nscan_erroronexit() {
	...
	if ( isset( $e['type'] ) && $e['type'] === E_ERROR ) {
		$err = str_replace( "\n", ' - ', $e['message'] );
		nscan_log_error( sprintf(
			__('Error: E_ERROR (%s - line %s)', 'ninjascanner'),
			$err,
			$e['line']
		));
...
function nscan_setup_antimalware( $lock_status ) {
	...
	if (! empty( $scan_signatures ) ) {
		...
		// Saved compiled signatures
		if ( $signatures_list ) {
			file_put_contents( NSCAN_TMP_SIGS, base64_encode( serialize( $signatures_list ) ) );
		} else {
...
function nscan_run_antimalware( $lock_status ) {
	...
	//Make sure we have some valid signatures
	if (! file_exists( NSCAN_TMP_SIGS ) ) {
		$message = $lock_status['message'];
		goto ANTIMALWARESAVE; // goto power!
	}

	$signatures = unserialize( base64_decode( file_get_contents( NSCAN_TMP_SIGS ) ) );
	if ( empty( $signatures ) ) {
