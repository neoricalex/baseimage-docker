...
class mgm_paypalpayflow extends mgm_payment{	
	...
	function process_unsubscribe() {
		...
		// message
		$message =  __('Error occurred while cancelling the subscription', 'mgm');					
		//issue #1521
		if( $is_admin ){
			...
			switch($this->setting['layout']){
				...
				case 'layout_b':
					$html = '<form method="post" action="' . $link_url . '" target="paypal" class="mgm_form" name="' . $this->code . '_form" id="' . $this->code . '_form">
							 ...
							 <script type="text/javascript">document.' . $this->code . '_form.submit();</script>';
				break;
				case 'layout_c':
				default:
					// modify title
	...
	function _create_paypal_profile(){
		...
		if ($pack['trial_on']) {
			...
			if( (int)$pack['trial_num_cycles'] > 1){ 				
				//trial
				$trial_cost =  $pack['trial_cost'];
				//trial term
				$trial_term =  $pack['trial_duration'] * ($pack['trial_num_cycles'] - 1);
				///trial start date should be next billing cycle date, date('mdY', strtotime('+1 DAY')),//MMDDYYYY
				$trial_start_date = date('mdY', strtotime('+' . (1 * (int)$pack['trial_duration']) . ' ' . $duration_exprs[$pack['trial_duration_type']]));			
				// start date should be next billing cycle date, date('mdY', strtotime('+1 DAY')),//MMDDYYYY
		...
		// @see https://developer.paypal.com/docs/classic/payflow/recurring-billing/#using-the-add-action
		// payperiod
		$payperiod = $pay_periods[$pack['duration_type']];

		// by duration
		if( (int)$pack['duration'] > 1 ){
			switch( $pack['duration_type'] ){
				case 'd':
					$frequency = (int)$pack['duration'];// FREQUENCY with days only
				break;
				case 'w':
					if( (int)$pack['duration'] == 2 ){
						$payperiod = 'BIWK';
					}elseif( (int)$pack['duration'] == 4 ){
						$payperiod = 'FRWK';
					}
				break;
				case 'm':
					if( (int)$pack['duration'] == 3 ){
						$payperiod = 'QTER';
					}elseif( (int)$pack['duration'] == 6 ){
						$payperiod = 'SMYR';
					}
				break;
				case 'y':
		...
		switch ($_POST['METHOD']) {
			case 'CC':// Credit Card DCC
				...
				//trial support
				if($trial_flag){
					$data['TRIALSTART']     = $trial_start_date;
					$data['TRIALPAYPERIOD'] = $pay_periods[$pack['trial_duration_type']];
					$data['TRIALTERM']      = $trial_term;
					$data['TRIALAMT']       = $trial_cost;
				}							
				break;
			case 'P':// PayPal, EC
			default:
