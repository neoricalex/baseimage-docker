	<form name="mgmexportfrm" id="mgmexportfrm" method="POST" action="admin-ajax.php?action=mgm_admin_ajax_action&page=mgm.admin.members&method=member_export">		
		<div class="table widefatDiv">
			...
			<div class="row brBottom">
				<div class="cell width150px">
					<b><?php _e('Payment Module','mgm') ?></b>
				</div>
				<div class="cell">
					<select name="bk_payment_module" onchange="this.form.bk_inactive.checked=(this.value!='all');" class="width200px">
						<option value="all"><?php _e('All','mgm') ?></option>
						<?php
						// init
						$strTypes = '';						
						// loop
						foreach(mgm_get_payment_modules_as_list() as $module_code => $module_name):
							// lang
							$module_name = __(mgm_stripslashes_deep($module_name), 'mgm');
							// append
							$strTypes .= sprintf('<option value="%s">%s</option>', $module_code, $module_name);
						endforeach;
						// print
						echo $strTypes;?>
					</select>
				...
				submitHandler: function(form) {   
					jQuery("#mgmexportfrm").ajaxSubmit({type: "POST",
					  ...
					  beforeSerialize: function(form, options){
						bk_only_selected = jQuery("#mgmexportfrm :input[name='bk_only_selected']").prop('checked');										
						if(bk_only_selected){
