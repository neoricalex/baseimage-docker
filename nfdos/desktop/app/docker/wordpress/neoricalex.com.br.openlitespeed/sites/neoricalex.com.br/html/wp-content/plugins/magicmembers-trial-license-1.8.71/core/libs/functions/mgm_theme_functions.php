		...
		if(empty($_POST)){
			...
			if( isset($_GET['action']) ){
				...
				if($_GET['action'] == 'complete_payment' && ! isset($_GET['show_other_packs'])){
					...
					$context    = 'complete-payment';
				}elseif($_GET['action'] == 'reactivate' ){
					$page_title = __('Reactivate Cancelled Subscription','mgm');
					$context    = 'reactivate-subscription';
				}else{
				// select
					$page_title = __('Select Membership Package','mgm');
					$context    = 'select-package-to-subscribe';
				}
			}	
		}	
	}	
	// return
	return apply_filters('mgm_payment_subscribe_page_title', $page_title, $context);	
}
...
function mgm_get_payment_subscribe_page_html(){
	...
	// msg
	$msg = __('You are already subscribed or an error occurred, Please contact administrator for more information.','mgm');
	// content
	...
	if ( ! empty($action) && count(mgm_get_class('membership_types')->membership_types) > 0 ){
		...
		// actions requires user logged in
		$actions_requires_login = array('upgrade','downgrade','reactivate','extend','purchase_another');

		// check 
		if( in_array($action, $actions_requires_login) ){
			if( is_user_logged_in() ){
				switch( $action ){
					// upgrade or downgrade, login required
					case 'upgrade':
					case 'downgrade':
					...
					case 'reactivate':
						$html = mgm_get_reactivate_buttons();
					break;
					// extend, login required	
					case 'extend':
						$html = mgm_get_extend_buttons();
					break;
					// purchase another, login required	
					case 'purchase_another':
						$html = mgm_get_purchase_another_subscription_buttons();
					break;
				}
			}else{
				$html = sprintf('<p>%s</p>', sprintf(__('Action - "<u>%s</u>" requires logged in user!','mgm'), $action));	
			}
		}else{
			switch( $action ){
				// complete_payment, temp login or toke auth
				case 'complete_payment':	
					$html = mgm_get_complete_payment_buttons();	
				break;
				// resubscribe, temp login or toke auth
				case 'resubscribe':
					$html = mgm_get_resubscribe_buttons();
				break;
				// bad action
				default:
					$html = sprintf('<p>%s</p>', sprintf(__('Error - Unknown action - "<u>%s</u>", Exiting...','mgm'), $action));	
				break;
			}	
		}											
	}else{
		...
		if( isset($user->ID) && $user->ID > 0 ) {
			...
			if( in_array($member->status, array(MGM_STATUS_NULL, MGM_STATUS_EXPIRED, MGM_STATUS_TRIAL_EXPIRED)) ){
				$html = mgm_get_subscription_buttons( $user );	
			}elseif( $member->status == MGM_STATUS_PENDING ){
				$msg = __('Error - Your subscription status is pending, Please contact administrator for more information.','mgm');
				$html = sprintf('<p>%s</p>', $msg);
			}
		}
	}	
...
function mgm_get_payment_processed_page_html(){
	...
	if(!empty($redirect_url) && $do_redirect){
		...
		if(!headers_sent()){ 				
			...
			$html .= sprintf('<script type="text/javascript">window.setTimeout(function(){window.location.href="%s";}, %d)</script>', $redirect_url, (int)$refresh_wait_time * 5);	
		}	
	}	
...
function mgm_get_transaction_page_html($return=false, $method=NULL){
	...
	switch($method){
		...
		case 'payment_reactivate_subscription': // reactivate handler
			// get module
			$module = mgm_request_var('module', '', true);
			// validate module
			if( $module_obj = mgm_is_valid_module($module, 'payment', 'object') ){
				// process, invoke process_return,process_notify,process_cancel,process_unsubscribe, process_subscription_reactivate
			...
			// not a valid module, call default for unsubscribe
				if( 'payment_unsubscribe' == $method){
				// default unsubscribe
					return mgm_member_unsubscribe();
				}else{
				// error
					return __('Invalid module!','mgm');
				}
			}								
		break;
