 ...
 class mgm_admin_members extends mgm_controller{
	...
	function member_list(){		
		...
		$data['search_fields'] = array(
			...
			'fee'=> __('Fee','mgm'), 'status'=> __('Status','mgm'), 'transaction_id'=> __('Transaction ID','mgm'), 
			'payment_module'=>__('Payment Module','mgm'), 'pack_id'=> __('Pack','mgm'),
			'subscr_id'=>__('Subscription Id','mgm')
		);
			...
			switch($search_field_name){
				...
				case 'reg_date':
					if(empty($search_field_value)){
						$search_field_value = date('Y-m-d',$timestamp);
					}
					if(empty($search_field_value_two)){
						$search_field_value_two = date('Y-m-d',$timestamp);
					}
				...
				case 'subscr_id':	
				// members
					$members    = mgm_get_members_with('subscr_id', $search_field_value);
					//super admins check
					$members = array_diff($members,$super_adminids);										
					// check
					$members_in = (count($members)==0) ? 0 : (implode(',', $members));
	...
	function subscription_packages_update(){
		...
		foreach($_POST['packs'] as $pack) {			
			...
			if($pack['duration_type'] == 'dr'){
				...
				// date formate  - issue #2775
				$duration_range_start_dt = str_replace('/', '-', $pack['duration_range_start_dt']);
				$duration_range_end_dt = str_replace('/', '-', $pack['duration_range_end_dt']);

				$pack['duration_range_start_dt'] = ((int)$pack['duration_range_start_dt'] > 0) ? date('Y-m-d', strtotime($duration_range_start_dt)) : '';
				$pack['duration_range_end_dt']   = ((int)$pack['duration_range_end_dt'] > 0) ? date('Y-m-d', strtotime($duration_range_end_dt)) : '';
			}else{
	...
	//Move role's users
	 ...
	 // get details
	private function _get_membership_details($member, $timestamp) {
		...
		if($bk_msexp_dur_unit && $bk_msexp_dur){
			...
			$date_diff   = strtotime($expire_date) - $timestamp;				
		...
		// membership type - issue #1431
		if (isset($bk_membership_type) && $bk_membership_type != 'all' ){
			//issue #1617
			if(strtolower(trim($member->membership_type)) != strtolower(trim($bk_membership_type)) 
				|| (isset($member->id ) && $member->id > 0 && is_super_admin( $member->id )))
			{
				return false;
			}
		}
		...
		// membership status
		if (isset($bk_membership_status) && $bk_membership_status != 'all' ){
			if($member->status != $bk_membership_status){
				return false;
			}
		}
		...
		//mgm_log( 'member: '. mgm_pr($member, true), __FUNCTION__ );

		// bk_payment_module
		if (isset($bk_payment_module) && $bk_payment_module != 'all' ){
			if(isset($member->payment_info->module) && $member->payment_info->module == $bk_payment_module){
				return true;
			}
		...
		//mgm_log( 'member: pass', __FUNCTION__ );
				...
				foreach ($members as $uid) {							
					...
					if((int)($uid)>0) {	
						...
						if(isset($user->ID) && (int)$user->ID > 0 ) {	
							...
							// manual 
							if(apply_filters('mgm_module_rebill_status', $user->ID, $member, 'manual')){								
								// update
								$updated++;	
							}							
							// update
	 ...
	 * New version of member export - (Due to performance issue this is implemented), 
	 * under testing process in customer site http://www.pathoma.com/.
	...
	function mgm_patch_partial_user_member_export($start, $limit, $cond) {
		...
		$sql = 'SELECT `u`.`ID`, `u`.`user_login`, `u`.`user_email`, `u`.`user_registered`,
		        `u`.`display_name`,`um`.`meta_value` 
		        FROM ' . $wpdb->users . ' u JOIN '.$wpdb->usermeta.' um	
			    ON `u`.`ID`=`um`.`user_id` WHERE `u`.`ID` <> 1 ' . $cond .' 
	...
	function member_export() {		
		...
		$mcf = mgm_get_class('member_custom_fields');
		...
		if(isset($export_member_info)){
			...
			if($date_start){
				...
				if($date_end){		
					...
					$query .= " AND UNIX_TIMESTAMP(`u`.`user_registered`) >= '{$date_start}' 
					            AND UNIX_TIMESTAMP(DATE_FORMAT(`u`.`user_registered`, '%Y-%m-%d')) <= '{$date_end}'";
				}else{
					$query .= " AND UNIX_TIMESTAMP(`u`.`user_registered`) >= '{$date_start}'";
				}
			}else if($date_end){
			...
			// timestamp
			$timestamp = mgm_get_current_datetime(null, false, 'timestamp');//time();	

			//issue #844 	
			$skip_fields = array(
				'subscription_introduction','coupon','privacy_policy','payment_gateways',
				'subscription_options','autoresponder','captcha','meta_value'
			);

			// check - issue #1382
			$rp_custom_fields = $mcf->get_fields_where(array('display'=>array('on_register'=>true,'on_profile'=>true)));
			
			$a_custom_fields = $mcf->get_fields_where(array('attributes'=>array('admin_only'=>true)));
			
			$custom_fields = array_merge($rp_custom_fields, $a_custom_fields);

			// check - issue #2440
			$cf_profile_by_membership_types = $mcf->get_fields_where(array('attributes'=>array('profile_by_membership_types'=>true)));
			...
			foreach (array_unique($mcf->sort_orders) as $id) {

				foreach($mcf->custom_fields as $field){
			...
			// sql
			$tr_sql = 'SELECT count(*) FROM ' . $wpdb->users . ' u JOIN '.$wpdb->usermeta.' um 
			           ON `u`.`ID` = `um`.`user_id` WHERE `u`.`ID` <> 1' . $query ;
			...
			if( $count ) {
				...
				for( $i = $start; $i < $count; $i = $i + $limit ) {
					...
					foreach ($users as $user) {		
						...
						if($this->_get_membership_details($member, $timestamp)) {							
							// merge defaults
							...
							foreach(get_object_vars($member_obj) as $field => $value){
								...
								if( isset($member->payment_info) ) {		
									// loop payments		
									foreach ($member->payment_info as $index => $val){	
										// skip
										if( 'txn_type' == $index ) continue;
										// field 
										$payf = 'payment_info_' . $index;	
										// set		
										$user->{$payf} = str_replace('mgm_', '', $val);		
									}		
								}	
							}							
							...
							// mgm_log( 'user: '.mgm_pr($user, true), __FUNCTION__ );
							...
							//mgm_log( 'user_i: '.mgm_pr($user_i, true), __FUNCTION__ );
						...
						if(isset($member->other_membership_types) && is_object($member->other_membership_types) && !empty($member->other_membership_types) ) {		
							...
							foreach ($member->other_membership_types as $key => $memtypes) {		
								...
								// check search parameters:		
								if($this->_get_membership_details($memtypes, $timestamp)) {		
									// copy		
			...
			$response = array(
				'status'  => 'error',
				'message' => __('Error while exporting members. Could not find any member with requested search parameters.', 'mgm')
			);
			// check
