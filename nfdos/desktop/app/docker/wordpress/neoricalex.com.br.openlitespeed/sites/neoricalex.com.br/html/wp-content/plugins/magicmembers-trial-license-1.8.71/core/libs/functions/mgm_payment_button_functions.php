...
/**
 * show buttons of modules available for complete_payment
 * *
 * @param array args
 * @return string html
 * @since 1.8.67
 */
function mgm_get_resubscribe_buttons($args=array()){
	global $wpdb;	
	// current user
	$user = wp_get_current_user();
	
	// get user from querystring, security check
	if( ! $user->ID ) {
	// get user from query string
		$user = mgm_get_user_by_querystring();// tested encoded
	}	
	...
	// bypass for now
	return mgm_get_upgrade_buttons( null, $user );
}
 ...
 * @since 1.8.67
 */
function mgm_get_complete_payment_buttons($args=array()){
	global $wpdb;	
	// current user
	$user = wp_get_current_user();
	...
	// action
	$__ptoken__ = mgm_get_var('__ptoken__', '', true);

	// set
	if( empty($__ptoken__) ){
		$__ptoken__ = mgm_get_auth_token($user->ID, 'user_pay_token');
	}	
	...
	// form action
	$url_params  = array('action' => $action, '__ptoken__'=>$__ptoken__);

	// form action
	$form_action = mgm_get_custom_url('transactions', false, $url_params);

	// cancel 
	$cancel_url = mgm_get_custom_url('login');
	...
	// save
	$save_form_flags = array('save_profile_back', 'save_profile_next');
	// show
	$show_form_flags = array('show_profile', 'show_payment');
	...
	if( ! empty($_POST)){
		...
		// save
		foreach( $save_form_flags as $save_form_flag ){
			if( isset($_POST[$save_form_flag]) && ! empty($_POST[$save_form_flag]) ){
				$save_form = $save_form_flag; 
			}
		}

		// save
		foreach( $show_form_flags as $show_form_flag ){
			if( isset($_POST[$show_form_flag]) && ! empty($_POST[$show_form_flag]) ){
				$show_form = $show_form_flag; 
			}
		}
	}

	//echo '<br>save_form: '. $save_form;

	// save form
	if( ! empty($save_form) ){
		// type
		switch( $save_form ){
			case 'save_profile_back':
			case 'save_profile_next':
				// show
				$show_form = 'default';
				if( 'save_profile_next' == $save_form ){
					$show_form = 'show_payment';
				}
	...
	if( ! empty($show_form) ){
		...
		switch( $show_form  ){
			case 'show_profile':
				...
				$button_save_profile = sprintf('<input class="button button-primary" type="submit" name="save_profile_back" value="%s" />', __('Save','mgm'));

				$button_save_next = sprintf('<input class="button button-primary" type="submit" name="save_profile_next" value="%s" />', __('Save & Next','mgm'));;

				$button_cancel = sprintf('<input class="button button-primary" type="button" name="cancel" onclick="window.location=\'%s\'" value="%s" />', $form_action, __('Cancel','mgm'));
				...
				$html .= sprintf('
					<p>
						...
						%s			 					
				 		...
				 		%s
			      	 	&nbsp;					
		            </p>', $button_save_profile, $button_save_next, $button_cancel);
				// html
				$html .= '</form>';
				// end
			break;
			...
			default:
				...
				// show error when no available packs
				if( ! $available_packages ){
					...
					// set	
						...
						// url
						$url_params = array('action' => $action, '__ptoken__' => $__ptoken__, 'edit_userinfo'=>1);
						// issue #1279
						$form_action = add_query_arg($url_params, mgm_get_current_url());			
					}
					...
					$form_id = 'mgm-form-user-complete-payment';
					// form
					$html .= sprintf('<form action="%s" method="post" class="mgm_form" name="%s" id="%s">', $form_action, $form_id, $form_id);
					$html .= sprintf('<div class="mgm_get_pack_form_container">%s', $available_packages);
					...
					$button_show_profile = sprintf('<input class="button button-primary" type="submit" name="show_profile" value="%s" />', __('Update Profile','mgm'));

					$button_show_payment = sprintf('<input class="button button-primary" type="submit" name="show_payment" value="%s" />', __('Complete Payment','mgm'));

					$button_cancel = sprintf('<input class="button button-primary" type="button" name="cancel" onclick="window.location=\'%s\'" value="%s" />', $cancel_url, __('Cancel','mgm'));
					...
					$html .= sprintf('
						<p>			
							%s	
						 	...
						 	%s
					      	&nbsp;
				        </p>', $button_show_profile, $button_show_payment, $button_cancel
			        );
...
/**
 * show buttons of modules available for reactivate
 * *
 * @param array args
 * @return string html
 * @since 1.8.67
 */
function mgm_get_reactivate_buttons($args=array()){
	global $wpdb;	
	...
	if( isset($member->payment_info->module ) && $module = $member->payment_info->module ) {
		...
		if( $obj_module = mgm_is_valid_module($module, 'payment', 'object') ){
			...
			if( $obj_module->is_reactivate_subscription_supported() ){
				return $obj_module->get_button_reactivate( $member );
			}
		}

		return sprintf('<p>%s</p>', sprintf(__('Module - "<u>%s</u>" does not support subscription reactivation!','mgm'), $module));
	}

	// default error
	return sprintf('<p>%s</p>', __('Unknown Error!','mgm'));	
}
...
function mgm_get_upgrade_buttons($args=array(), $user=null) { 
	...
	if( isset($_POST['submit']) ) {	
		...
		// save member data including coupon etc, MUST save after all validation passed, we dont want any 
		// unwanted value in member object unless its a valid upgrade			
		$action_t = strip_tags($action);
		// save
		if ($multiple_upgrade) {
			$member = mgm_save_partial_fields(array('on_upgrade'=>true),'mgm_upgrade_field', $selected_pack['cost'], true, $action_t, 
				$member);
		}else {
			$member = mgm_save_partial_fields(array('on_upgrade'=>true),'mgm_upgrade_field', $selected_pack['cost'], true, $action_t);
		}
		...
		if( ! is_null($cf_payment_gateways) ) {				
			// get pack				
			mgm_get_upgrade_coupon_pack($member, $selected_pack, $action_t);				
			// cost
			if ((float)$selected_pack['cost'] > 0) {
				//get an object of the payment gateway:
				$mod_obj = mgm_get_module($cf_payment_gateways,'payment');
		...
		// get coupon pack
		mgm_get_upgrade_coupon_pack($member, $selected_pack, strip_tags($action));			
		// start html
		$html = '<div>';
		// free package
		if (($selected_pack['cost'] == 0 || $selected_pack['membership_type'] == 'free') && in_array('mgm_free', $a_payment_modules) && mgm_get_module('mgm_free')->enabled=='Y') {	
 ...
 */ 
function mgm_get_extend_buttons() {
	global $wpdb;    
	...
	// generate form ----------------------------------------------------------------
		...
		if($error){
			...
			if(!$extend_packages){
				...
				// html
				// $html .= '<input type="hidden" name="ref" value="'. mgm_get_pack_ref($member) .'" />';					
				$html .= '<input type="hidden" name="form_action" value="'. $form_action .'" />';					
...
/**
 * create purchase another subscription buttons
 *
 * @param array userdata
 * @return string html output
 */
function mgm_get_purchase_another_subscription_buttons($args = array()) {
	global $wpdb;	
	...
	$packs           = $packs_obj->get_packs('upgrade',true,array(0));
	$duration_str    = $packs_obj->duration_str;
	$trial_taken     = $member->trial_taken;	
	// pack_ids	
...
function mgm_get_post_purchase_buttons(){
	...
	// post pack purchase
		...
		// set pack
		$pack = array(
			'duration'=>1,'item_name'=>$item_name,'buypost'=>1,'cost'=>$cost,'title'=>$postpack->name, 
