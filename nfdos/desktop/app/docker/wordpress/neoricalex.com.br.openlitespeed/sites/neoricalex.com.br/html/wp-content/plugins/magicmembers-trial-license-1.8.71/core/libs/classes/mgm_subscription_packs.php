...
class mgm_subscription_packs extends mgm_object{
	 ...
	 * @return array $packs
	 */
	public function get_all($page='all', $sort=true, $package=array(), $pack_id=null){
		// init
		...
		foreach ($this->packs as $_pack) {			
			...
			if($page != 'all') { 	
				// not active, issue #1248 
				if(! isset($_pack['active'][$page]) || 
					( isset($_pack['active'][$page]) && (bool)$_pack['active'][$page] == FALSE) ) {
					//Skip if not active
					continue;	
				}
				...
				if($page == 'register'){
					...
					if( isset($_pack['hidden']) && (bool)$_pack['hidden'] == TRUE ){
						// only if package or pack id selected, iss#2754
						if( (isset($package['id']) && (int)$package['id'] == $_pack['id'])
						|| (! is_null($pack_id) && (int)$pack_id == $_pack['id']) ) {
							//
						}else{
							continue;
						}	
	 ...
	 */	
	function get_packs($page='all', $sort=true, $package=array(), $pack_id=null){		
		// use 
		return $this->get_all($page, $sort, $package, $pack_id);
	}
