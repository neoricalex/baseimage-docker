...
function mgm_shortcode_parse($args, $content, $tag) {	
	...
	switch ($tag) {
		...
		case 'user_purchase_another_membership':
			...
			$content = mgm_get_purchase_another_subscription_buttons($args);
		break;
		case 'user_subscribe':
		case 'user_register':					
			// named
		...
		break;	
		// issue, this code replaces user_login in user object?
		case 'user_login':
			// user login form
			$content = mgm_user_login_form(false);
		break;		
	...
	switch ($function) {		
		...
		case 'user_account_is':
			...
			if (!$current_user && isset($_GET['token']) && mgm_use_rss_token()) {
				// get user
				$current_user = mgm_get_user_by_rss_token(strip_tags($_GET['token']));
			}
			// set 
			...
			if(is_super_admin()) {
				...
				if (mgm_is_member_active($user_id) && array_diff($membership_types, $user_membership_types) != $membership_types) { 
				// if any match found
					$return = $matches;
				}
			}else {
